...
Private Const HEADER_CG As String = "CG"
Private Const HEADER_REF As String = "Ref"
Private Const HEADER_IMP_DATE As String = "Imp Date"
Private Const HEADER_DESC As String = "Desc"
Private Const HEADER_APP_REF As String = "App Ref"
Private Const HEADER_OUTCOME As String = "Outcome"
Private Const HEADER_CONF As String = "Conf"
...
Private Sub ReviewReportData(reportWs As Worksheet, reviewWs As Worksheet, keywordsWs As Worksheet)
    ...
    ' Log missing headers
    If cgCol = 0 Then LogIssue reviewWs, GetNextIssueRow(reviewWs), "Missing Header", HEADER_CG, 0, "", HEADER_CG & " column header not found"
    If refCol = 0 Then LogIssue reviewWs, GetNextIssueRow(reviewWs), "Missing Header", HEADER_REF, 0, "", HEADER_REF & " column header not found"
    If impDateCol = 0 Then LogIssue reviewWs, GetNextIssueRow(reviewWs), "Missing Header", HEADER_IMP_DATE, 0, "", HEADER_IMP_DATE & " column header not found"
    If descCol = 0 Then LogIssue reviewWs, GetNextIssueRow(reviewWs), "Missing Header", HEADER_DESC, 0, "", HEADER_DESC & " column header not found"
    If appRefCol = 0 Then LogIssue reviewWs, GetNextIssueRow(reviewWs), "Missing Header", HEADER_APP_REF, 0, "", HEADER_APP_REF & " column header not found"
    If outcomeCol = 0 Then LogIssue reviewWs, GetNextIssueRow(reviewWs), "Missing Header", HEADER_OUTCOME, 0, "", HEADER_OUTCOME & " column header not found"
    If confCol = 0 Then LogIssue reviewWs, GetNextIssueRow(reviewWs), "Missing Header", HEADER_CONF, 0, "", HEADER_CONF & " column header not found"
    ...
    If lastRow < 2 Then
        LogIssue reviewWs, GetNextIssueRow(reviewWs), "No Data", "N/A", 0, "", "No data rows found in report"
        Exit Sub
    End If
    ...
    For i = 2 To lastRow
        ...
        ValidateCellFormatting reportWs, reviewWs, i, nextIssueRow
        
        ' Rule 0b: Check for blank cells within used range
        ValidateBlankCells reportWs, reviewWs, i, nextIssueRow
        
        ' Rule 1: Check CG column
        If cgCol > 0 Then
            ValidateCGColumn reportWs, reviewWs, i, cgCol, nextIssueRow
        End If
        
        ' Rule 2: Check Ref column
        If refCol > 0 Then
            ValidateRefColumn reportWs, reviewWs, i, refCol, nextIssueRow
        End If
        
        ' Rule 3: Check Imp Date column
        If impDateCol > 0 Then
            ValidateImpDateColumn reportWs, reviewWs, i, impDateCol, nextIssueRow
        End If
        
        ' Rule 4: Check Desc column (spell check and forbidden terms)
        If descCol > 0 Then
            ValidateDescColumn reportWs, reviewWs, keywordsWs, i, descCol, nextIssueRow, forbiddenTerms
        End If
        
        ' Rule 5: Check App Ref vs CG dependency
        If cgCol > 0 And appRefCol > 0 Then
            ValidateAppRefColumn reportWs, reviewWs, i, cgCol, appRefCol, nextIssueRow
        End If
        
        ' Rule 6: Check Outcome column
        If outcomeCol > 0 Then
            ValidateOutcomeColumn reportWs, reviewWs, i, outcomeCol, nextIssueRow
        End If
        
        ' Rule 7: Check Conf column
        If confCol > 0 Then
            ValidateConfColumn reportWs, reviewWs, i, confCol, nextIssueRow
        End If
    Next i
...
Private Sub ValidateCellFormatting(ws As Worksheet, reviewWs As Worksheet, rowNum As Long, ByRef nextRow As Long)
    Dim lastCol As Long
    Dim colNum As Long
    ...
    For colNum = 1 To lastCol
        ...
        If hasLeadingTrailingWS Or hasNewline Or hasDoubleSpace Then
            ...
            LogIssue reviewWs, nextRow, "Formatting Issue", colHeader, rowNum, cellText, issuesList
            nextRow = nextRow + 1
        End If
...
Private Sub ValidateBlankCells(ws As Worksheet, reviewWs As Worksheet, rowNum As Long, ByRef nextRow As Long)
    Dim lastCol As Long
    Dim colNum As Long
    Dim cellValue As Variant
    Dim cellText As String
    ...
    For colNum = 1 To lastCol
        ...
        If cellText = "" Then
            colHeader = Trim(CStr(ws.Cells(1, colNum).Value))
            If colHeader = "" Then colHeader = "Column " & colNum
            LogIssue reviewWs, nextRow, "Blank Cell", colHeader, rowNum, "", "Cell is blank within used range"
            nextRow = nextRow + 1
        End If
    Next colNum
End Sub
...
Private Sub ValidateCGColumn(ws As Worksheet, reviewWs As Worksheet, rowNum As Long, colNum As Long, ByRef nextRow As Long)
    Dim cellValue As String
    ...
    If cellValue <> "" Then
        If cellValue <> "Non Sub" And cellValue <> "RN" Then
            LogIssue reviewWs, nextRow, "Invalid CG", HEADER_CG, rowNum, cellValue, HEADER_CG & " must be 'Non Sub' or 'RN'"
            nextRow = nextRow + 1
        End If
    End If
End Sub
...
Private Sub ValidateRefColumn(ws As Worksheet, reviewWs As Worksheet, rowNum As Long, colNum As Long, ByRef nextRow As Long)
    Dim cellValue As String
    ...
    If cellValue <> "" Then
        ...
        If Not isValid Then
            LogIssue reviewWs, nextRow, "Invalid Ref", HEADER_REF, rowNum, cellValue, HEADER_REF & " must start with CRQ followed by 6 or 7 digits"
            nextRow = nextRow + 1
        End If
    End If
End Sub
...
Private Sub ValidateImpDateColumn(ws As Worksheet, reviewWs As Worksheet, rowNum As Long, colNum As Long, ByRef nextRow As Long)
    Dim cellValue As Variant
    Dim cellText As String
    ...
    If cellText <> "" Then
        If Not IsDate(cellValue) Then
            LogIssue reviewWs, nextRow, "Invalid Imp Date", HEADER_IMP_DATE, rowNum, cellText, HEADER_IMP_DATE & " must be a valid date (mm/dd/yyyy)"
            nextRow = nextRow + 1
        End If
    End If
End Sub
...
Private Sub ValidateDescColumn(ws As Worksheet, reviewWs As Worksheet, keywordsWs As Worksheet, rowNum As Long, colNum As Long, ByRef nextRow As Long, forbiddenTerms As Variant)
    Dim cellValue As String
    ...
    ' Check for forbidden terms (case-sensitive)
    If IsArray(forbiddenTerms) Then
        For Each term In forbiddenTerms
            If InStr(1, descText, CStr(term), vbBinaryCompare) > 0 Then
    ...
    If hasSpellingIssue Then
        LogIssue reviewWs, nextRow, "Spelling", HEADER_DESC, rowNum, misspelledWords, "Potential spelling issues found in " & HEADER_DESC
        nextRow = nextRow + 1
    End If
    
    ' Log forbidden term issues
    If hasForbiddenTerm Then
        LogIssue reviewWs, nextRow, "Forbidden Term", HEADER_DESC, rowNum, matchedTerms, "Forbidden term(s) found in " & HEADER_DESC
        nextRow = nextRow + 1
    End If
End Sub

' Validate App Ref column based on CG value
Private Sub ValidateAppRefColumn(ws As Worksheet, reviewWs As Worksheet, rowNum As Long, cgCol As Long, appRefCol As Long, ByRef nextRow As Long)
    ...
    If cgValue = "Non Sub" Then
        ...
        If appRefText <> "N/A" Then
            LogIssue reviewWs, nextRow, "CG/App Ref Mismatch", HEADER_APP_REF, rowNum, appRefText, "When " & HEADER_CG & " is 'Non Sub', " & HEADER_APP_REF & " must be 'N/A'"
            nextRow = nextRow + 1
        End If
    ElseIf cgValue = "RN" Then
        ...
        If appRefText <> "" Then
            If Not IsDate(appRefValue) Then
                LogIssue reviewWs, nextRow, "CG/App Ref Mismatch", HEADER_APP_REF, rowNum, appRefText, "When " & HEADER_CG & " is 'RN', " & HEADER_APP_REF & " must be a valid date"
                nextRow = nextRow + 1
            End If
        Else
            LogIssue reviewWs, nextRow, "CG/App Ref Mismatch", HEADER_APP_REF, rowNum, "", "When " & HEADER_CG & " is 'RN', " & HEADER_APP_REF & " must contain a date"
            nextRow = nextRow + 1
        End If
    End If
End Sub
...
Private Sub ValidateOutcomeColumn(ws As Worksheet, reviewWs As Worksheet, rowNum As Long, colNum As Long, ByRef nextRow As Long)
    Dim cellValue As String
    ...
    If cellValue <> "" Then
        If cellValue <> "Successful" Then
            LogIssue reviewWs, nextRow, "Invalid Outcome", HEADER_OUTCOME, rowNum, cellValue, HEADER_OUTCOME & " must be 'Successful'"
            nextRow = nextRow + 1
        End If
    End If
End Sub
...
Private Sub ValidateConfColumn(ws As Worksheet, reviewWs As Worksheet, rowNum As Long, colNum As Long, ByRef nextRow As Long)
    Dim cellValue As String
    ...
    If cellValue <> "" Then
        If cellValue <> "No" Then
            LogIssue reviewWs, nextRow, "Invalid Conf", HEADER_CONF, rowNum, cellValue, HEADER_CONF & " must be 'No'"
            nextRow = nextRow + 1
        End If
    End If
End Sub
...
Private Sub LogIssue(ws As Worksheet, rowNum As Long, issueType As String, colName As String, reportRow As Long, valueText As String, details As String)
    ws.Cells(rowNum, 1).Value = issueType
    ws.Cells(rowNum, 2).Value = colName
    ws.Cells(rowNum, 3).Value = reportRow
